...
describe("UserController", () => {
  ...
  describe("getUserByUsername", () => {
    test("getUserByUsername should throw an invalid parameter error", () => {
      expect(controller.getUserByUsername(userCustomer, "")).rejects.toThrow(
        InvalidParametersError
      );
    });
    ...
    test("getUserByUsername should throw an unhauthorized error", () => {
      expect(
        controller.getUserByUsername(userCustomer, "test2")
    ...
    test("getUserByUsername should return a user by username", async () => {
      jest
        ...
        .mockResolvedValueOnce(userCustomer);
      await expect(
        controller.getUserByUsername(userCustomer, "test1")
      ).resolves.toEqual(userCustomer);
  ...
  describe("getUsersByRole", () => {
    test("getUsersByRole should return all users with a specific role", async () => {
      jest
        ...
        .mockResolvedValueOnce([userCustomer]);
      await expect(controller.getUsersByRole("Customer")).resolves.toEqual([
        userCustomer,
      ]);
    ...
    test("It should throw an invalid role error", async () => {
      expect(controller.getUsersByRole("Not valid")).rejects.toThrow(
        InvalidRoleError
      );
    });
  });
  ...
  describe("deleteUser", () => {
    test("deleteUser should throw an unhauthorized error", () => {
      expect(controller.deleteUser(userCustomer, "test2")).rejects.toThrow(
        UnauthorizedUserError
      );
    });
    ...
    test("It should delete a user", async () => {
      ...
      await expect(controller.deleteUser(userCustomer, "test1")).resolves.toBe(
        true
      );
    ...
    test("It should delete a user as admin", async () => {
      jest
        .spyOn(UserDAO.prototype, "deleteUserAsAdmin")
        .mockResolvedValueOnce(true);
      await expect(controller.deleteUser(userAdmin, "test1")).resolves.toBe(
        true
      );
    ...
    test("It should throw InvalidParametersError if the user is not found", async () => {
      ...
      await expect(controller.deleteUser(userCustomer, "")).rejects.toThrow(
        InvalidParametersError
      );
      expect(UserDAO.prototype.deleteUser).toHaveBeenCalledTimes(0);
    });
  ...
  describe("deleteAllUsers", () => {
    test("deleteAllUsers should delete all users", async () => {
      jest
        .spyOn(UserDAO.prototype, "deleteAllUsers")
        .mockResolvedValueOnce(true);
  ...
  describe("updateUserInfo", () => {
    test("updateUserInfo should update a user's information", async () => {
      jest
        ...
        .mockResolvedValueOnce(userCustomerNew);
      await expect(
      ...
      ).resolves.toEqual(userCustomerNew);
    ...
    test("updateUserInfo should update a user's information as an admin", async () => {
      jest
        .spyOn(UserDAO.prototype, "updateUserAsAdmin")
        .mockResolvedValueOnce(userCustomerNew);
