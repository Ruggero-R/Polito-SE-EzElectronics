...
beforeEach(async () => {
  ...
  await ProductDao.prototype.registerProducts(
    "Model",
    "Laptop",
    32,
    null,
...
describe("Integration tests between DAO and DB for the review component", () => {
  /* *********************************************** *
  ...
  describe("Integration tests for the deleteReview method", () => {
    ...
    test("It should not find any review to delete due to an unexisting pair (model-user)", async () => {
      await expect(RevDAO.deleteReview("Model2", Utente1)).rejects.toThrow(
        NoReviewProductError
      );
    });
  ...
  describe("Integration tests for the deleteReviewsOfProduct method", () => {
    test("It should not find any review to delete due to an unexisting model", async () => {
      await expect(RevDAO.deleteReviewsOfProduct("iPhone13")).rejects.toThrow(
        ProductNotFoundError
      );
    });
